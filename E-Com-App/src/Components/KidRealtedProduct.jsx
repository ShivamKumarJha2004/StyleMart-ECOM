...
const RelatedProduct = ({ category }) => {
  ...
  // Fetch related products when the component mounts or when category changes
  useEffect(() => {
    const fetchRelatedProducts = async () => {
      try {
        const response = await fetch('https://stylemart-ecom-backend.onrender.com/api/relatedProduct', {
          method: 'POST', // Ensure POST since you're sending the category
          headers: {
            'Content-Type': 'application/json',
          },
          body: JSON.stringify({ category }), // Sending the selected category
        });
